  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-list">
        {loading ? <p className="admin-empty">Carregando fichas...</p> : sheets.length === 0 ? <p className="admin-empty">Nenhuma ficha foi salva ainda.</p> : sheets.map((sheet) => <article className="admin-row" key={sheet.id}><div><strong>{sheet.data?.characterName || "Personagem sem nome"}</strong><span>{sheet.owner_email}</span></div><div className="admin-row-actions"><time>{new Date(sheet.updated_at).toLocaleString("pt-BR")}</time><button className="admin-button" onClick={() => setSelected(sheet)}>abrir ficha</button></div></article>)}
      </section>
